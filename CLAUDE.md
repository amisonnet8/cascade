...
| `amivm/amivm_spec.md` | AMIVM-IRの唯一の正確な仕様(下記「AMIVM-IRの書き方」節に要点を転記)。amivm本体のバージョンを上げた際は齟齬がないか確認すること |
...
以下は`amivm/amivm_spec.md`からの要点転記。**Cascadeのコード生成部がAMIVM-IRを出力する際は、この命令セット・カテゴリ・Kind分類に厳密に従うこと。**

### 制約・前提条件

- `FUNC`・`FUNCM`はトップレベルのみに置ける(関数のネスト不可)。`STTYPE`・`INTYPE`もネスト不可。**`IF`・`LOOP`・`CLOS`・`SEL`はいずれもネストできる**(互いの本体の中に`IF`/`LOOP`/`CLOS`/`SEL`を任意の組み合わせ・任意の深さで書ける。`CLOS`のネスト深さ`L`は`FUNC`直下を1として数え、クロージャー引数`&L-N`の階層番号に対応する)
...
- `FUNC`/`FUNCM`/`CALL`/`DEFER`/`SPAWN`/`STTYPE`/`INTYPE`は、いずれもオプションの型パラメータ・明示的型引数(`<<typename constraint ... :>>`/`<<type1 type2 ... :>>`)を伴うジェネリクス構文を取れる。**Cascadeの言語仕様(`cascade_spec.md`)にジェネリクス相当の機能が無いため、Cascade側はこの拡張構文を一切使わない**(省略時は従来どおりの非ジェネリクス構文のままで、この拡張は後方互換——下記「命令使用ゴール」参照)

### 識別子のプレフィックス

| 記号 | 意味 |
|---|---|
| `$` | 関数引数(`FUNCM`本体内に限り`$0`でレシーバーも参照できるが、Cascadeはレシーバーを普通の`$1`として扱うため`$0`は使わない——下記「命令使用ゴール」参照) |
| `&` | クロージャー引数(`&N`は自分がいる`CLOS`階層のN番目、`&L-N`で階層`L`を明示指定できる) |
| `%` | 関数内変数名 |
| `@` | 関数外変数名(グローバル変数) |
| `^` | 型名 |
| `>` | 構造体フィールド名 |
| `<` | メソッド名(`METHVAL`・`INTYPE`内の`METHOD`で使う。Cascadeは未使用) |
...
| メソッド値・関数値取得 | `METHVAL local variable method` / `FUNCVAL local callname`(`:=`で新規宣言しながら取得する専用命令。Cascadeは未使用——下記「命令使用ゴール」参照) |
| メソッド定義 | `FUNCM defname receiver type1 ... : type3 ...` / `RET`(`FUNC`と共用) / `ENDFUNCM`(Goネイティブのレシーバー付きメソッド。Cascadeは使わず、レシーバーを第一引数とする普通の`FUNC`で代替している——下記「命令使用ゴール」参照) |
...
| `METHVAL` `FUNCVAL` | 対応するCascade機能なし。Goのメソッド値・関数値を`:=`で取得する専用命令だが、Cascade組み込みのレシーバー関数はコンパイル時に静的解決できるため`METHVAL`(Step8の判断のまま)は不要。`FUNCVAL`はStep9で意図的に未サポートと確定した「トップレベル関数を値として渡す」機能を将来実装する際の有力な手段になりうるが、対応するCascade言語機能自体が今も存在しないため、現時点では対象外(下記「確定した設計判断」参照) | 対象外 |
| `FUNCM` `ENDFUNCM` | 対応するCascade機能なし(現状)。レシーバー付き関数(`obj.method()`)自体はCascadeの中核機能(3本柱の1つ)だが、Step8で確定した「レシーバーを第一引数とする普通のFUNC」という実装方針は新しいamivmでも無変更で動作し続けることを実地検証済み。`FUNCM`への切り替えはGoネイティブのメソッド値・インターフェース実装等、Cascadeが現状持たない機能と組み合わさって初めて価値が出るため、今回は見送った(下記「確定した設計判断」参照) | 対象外 |
| `INTYPE` `METHOD` `ENDINTYPE` | 対応するCascade機能なし。Goのインターフェース型に相当するが、Cascadeの型システムに`any`相当・多態(ポリモーフィズム)の概念が無く、`ASSERT`と同じ理由で意図的に対象外とする | 対象外 |
| `GETYPE` | 対応するCascade機能なし。ジェネリクス型に型引数を当てはめて別名宣言する命令だが、Cascadeの言語仕様(`cascade_spec.md`)自体にジェネリクスが無く、意図的に対象外とする | 対象外 |
...
### AMIVM命令改修・第5弾(METHVAL/FUNCVAL/FUNCM/INTYPE/GETYPE追加、ジェネリクス対応)の評価と「コード変更不要」の確認

amivm本体に、Goのジェネリクス(型パラメータ・明示的型引数)対応と、それに付随する5つの新命令(`METHVAL`/`FUNCVAL`/`FUNCM`/`ENDFUNCM`/`INTYPE`/`METHOD`/`ENDINTYPE`/`GETYPE`——実質7命令)が一括で追加された。加えて`amivm/docs/`配下だった仕様ドキュメントがリポジトリ直下(`amivm/amivm_spec.md`)へ、`amivm/test_ir/`が`amivm/examples/`へ、それぞれ移動された(amivm自身のリファクタリング。参照パスのみの変更で仕様内容とは無関係)。

**結論として、Cascade側のコード変更は一切不要だった。** 既存13サンプル(`examples/01〜13_*.cas`)+`examples/14_packages`の全てを、新しくインストールしたamivmで`cascade run`(内部で`amivm`→`go build`→実行のパイプラインを通す)にかけ、無変更のまま全て成功することを確認した(実行結果も従来どおり)。理由は単純で、今回の改修が追加した構文(型パラメータ・明示的型引数・`FUNCM`/`INTYPE`/`METHVAL`/`FUNCVAL`/`GETYPE`)はいずれも**既存命令へのオプション追加**であり、使わなければ従来どおりの非ジェネリクス構文がそのまま通る後方互換設計だったため(`amivm_spec.md`の`FUNC`/`STTYPE`等の構文が`<<...>>`という省略可能な区画として型パラメータを追加している点を参照)。`go test ./...`も無変更で全て成功した。

**新命令5つ(実質7命令)は、いずれも対応するCascade言語機能が存在しないため「命令使用ゴール」表で対象外と記録した(`ASSERT`と同じ扱い)。** 個別の判断根拠は以下のとおり。

- **`INTYPE`/`METHOD`/`ENDINTYPE`(インターフェース)・`GETYPE`(ジェネリクス実体化)**: Cascadeの型システムには`any`相当の型・多態(ポリモーフィズム)・ジェネリクスのいずれも存在しない(`cascade_spec.md`に該当する構文が無いことを`grep`で確認済み)。`ASSERT`のときと全く同じ理由・同じ判断で対象外とする。
- **`METHVAL`**: Cascade組み込みのレシーバー関数はコンパイル時に静的解決できる(Step8で確定済み)ため、値としての動的取得を要する場面が無い。この判断は変わらない。
...
- **作業のsummary(要約)とユーザーへの質問は日本語で行うこと。**
